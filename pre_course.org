...
:PROPERTIES:
:ID:       2f4ffbe3-1f86-4612-88c6-86cf5971798b
:END:
- *P (Polynomial) Problems:* problems that can be solved in polynomial time and whose solutions can be verified in polynomial time are called *problems in P*.
  - e.g. Sorting a list : e.g. mergesort takes $\mathcal{O}(n \cdot log(n))< \mathcal{O}(n^{2})$ to sort a list and $\mathcal{O}(n)$ to verify that the list is sorted.
...
** IN-PROGRESS Group Generator
:PROPERTIES:
:ID:       4169039c-64bf-435f-afd4-bd8b7c7a0e9b
:END:

- in a cyclic group of order $n$ any element of order n is a generator
- in a cyclic group, the number of generators equals the number of integers k such that gcd(k,n)=1 (That count is the [[id:108230aa-cb69-4484-8afa-92af3f6c0818][Euler's totient function]]).

See also [[id:f5f80632-6c31-4065-9470-33d7ea93c025][Cyclic groups]]
*** TODO Example: $(Z_{10},*)$
...
Cryptographic protocols often assume the existence of finite cyclic groups of prime order.
However real-world implementations of those protocols are not defined on prime order groups, but on groups where the order consist of a (usually large) prime number that has small [[id:5cf1c4ca-35a9-425c-8fe9-ccdbd78d7d4d][Cofactors]]. 
In this case, a method called [[id:717783f4-2954-4d7b-8d44-8fcb8f877834][Cofactor Clearing]] has to be applied to ensure that the computations are not done in the group itself but in its (large) prime order subgroup.
