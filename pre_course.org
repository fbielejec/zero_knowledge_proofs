...
** TODO Demonstrate that $g^{n} \pmod p, n = \ldots -2, -1, 0, 1, 2 \ldots$, where $g$ and $p$ are relatively prime is a group under multiplication. That is, given elements $g^a$, $g^b$, $(g^a) \cdot (g^b)$ is in the group and the binary operator follows the group laws.
