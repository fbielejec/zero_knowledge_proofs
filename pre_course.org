...
=Define our set $A$ to be the numbers ${0,1,2,3,4}$ and our binary operator to be subtraction modulo 5. Define all the ordered pairs of $A \times A$ in a table, then map that set of ordered pairs to $A$.=
TODO
...
=Show that the set $\{1\}$ with the binary operator $\times$ is a group.=
...
=Integers (positive and negative) are not a group under multiplication. Show which of the four requirements (closed, associative, existence of identity, all elements having an inverse) are not satisfied=
...
=Suppose you have a set F={monyet, kodok, burung, ular}. Define a binary operator that turns it into a group using set-theoretic definitions=
...
(monyet * monyet) * monyet = monyet * (monyet * monyet) =
(monyet * monyet) * kodok  = monyet * (monyet * kodok)  = monyet * monyet
(monyet * monyet) * burung = monyet * (monyet * burung) = moneyt * kodok = monyet
(monyet * monyet) * ular   = monyet * (monyet * ular)   =

(monyet * kodok) * monyet = monyet * (kodok * monyet) =
(monyet * kodok) * kodok  = monyet * (kodok * kodok)  = monyet * kodok = monyet
(monyet * kodok) * burung = monyet * (kodok * burung) =
(monyet * kodok) * ular   = monyet * (kodok * ular)   =

(monyet * burung) * monyet = monyet * (burung * monyet) =
(monyet * burung) * kodok  = monyet * (burung * kodok)  = monyet * burung
(monyet * burung) * burung = monyet * (burung * burung) =
(monyet * burung) * ular   = monyet * (burung * ular)   = monyet * kodok = monyet

(monyet * ular) * monyet = monyet * (ular * monyet) = monyet * kodok = monyet
(monyet * ular) * kodok  = monyet * (ular * kodok)  = monyet * ular = 
(monyet * ular) * burung = monyet * (ular * burung) = 
(monyet * ular) * ular   = monyet * (ular * ular)   = 



- Identity element: $kodok$
- Inverse:
