...
=Suppose you have a set A={monyet, kodok, burung, ular}. Define a binary operator that turns it into a group using set-theoretic definitions=
...
(monyet * burung) * ular   = monyet * (burung * ular)
(kodok  * burung) * ular   = kodok  * (burung * ular) = kodok * kodok = kodok
(ular   * monyet) * kodok  = ular   * (monyet * kodok) = ular * monyet = kodok
