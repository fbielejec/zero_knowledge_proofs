...
- sage: operators &, |, ~, ^, ->, <->, corresponding to and, or, not, xor, if...then, if and only if.
...
=Example: checking whether A > B (in 4 bit logic)=
...
#+BEGIN_SRC jupyter-python :session zk :async yes
  ...
  def compare(bits_p, bits_q):
      ...
      Assumes BigEndian representation (MSB at index 0)
