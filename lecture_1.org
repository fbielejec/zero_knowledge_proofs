...
We can extend it to compute $a^n \quad \text{m}$ to e.g. compute [[*Multiplicative inverses in a Finite Field][Multiplicative inverses in a Finite Field]].
...
fn binary_exponentiation_mod(a: u32, n: i32, m: u32) -> u32 {
    ...
    if n < 0 {
        // (1/a)^-n but in modular arithmetic
